...
export default function ContactPageClient({ lang, prefillProduct = '' }: ContactPageClientProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setInquiryNumber(inqNum);
    setSubmitted(true);

    // Save to local history
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      const history: InquiryRecord[] = saved ? JSON.parse(saved) : [];
            ...
            <div className="card" style={{ padding: '32px' }}>
              {submitted ? (
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <button type="submit" className="btn-gold" style={{ width: '100%', justifyContent: 'center', padding: '14px', fontSize: 15 }}>
                    📨 {m.contact.submit} →
                  </button>
